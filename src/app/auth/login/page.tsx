 ...
 *
 * Two-step authentication flow mimicking X's login:
 *   Step 1 ("identifier"): Enter email or username
 *     - Disabled social provider buttons (phone, Google, Apple) for UI parity
 *     - "Continue" button moves to step 2
 *     - Link to registration page
 *   Step 2 ("password"): Enter password
 *     - Show/hide password toggle
 *     - "Back" button returns to step 1
 *     - Submits via NextAuth credentials signIn()
 *
 * On successful login, redirects to home ("/").
 * On error, displays "Wrong email, phone, or password."
 *
...
export default function SignInPage() {
  ...
  // Step 1: validates identifier is non-empty, then shows password input
  ...
  // Step 2: calls NextAuth credentials signIn, handles error/success
  ...
  return (
    <div className="min-h-screen bg-black text-white flex">
        ...
        {step === "identifier" ? (
          ...
          <>
            <div className="flex flex-col gap-3 w-full max-w-[300px]">
              {/* Disabled social buttons (UI placeholders) */}
              <button type="button" className="opacity-30 flex items-center justify-center gap-3 w-full rounded-full border border-neutral-700 bg-white text-black font-semibold text-[15px] py-2.5 px-4 hover:bg-neutral-100 transition-colors">
                <svg className="h-5 w-5" viewBox="0 0 24 24" fill="currentColor"><path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56a.977.977 0 0 0-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z" /></svg>
                Continue with phone
              </button>
              <button type="button" className="opacity-30 flex items-center justify-center gap-3 w-full rounded-full border border-neutral-700 bg-white text-black font-semibold text-[15px] py-2.5 px-4 hover:bg-neutral-100 transition-colors">
              ...
              <form onSubmit={handleContinue} className="flex flex-col gap-4">
                ...
                <button type="submit" className="w-full rounded-full bg-neutral-200 text-black font-bold text-[17px] py-2.5 hover:bg-white transition-colors">
                  Continue
                </button>
              </form>
          ...
          <div className="flex flex-col gap-3 w-full max-w-[300px]">
            <p className="text-neutral-400 text-sm mb-1">
              Signing in as <strong className="text-white">{identifier}</strong>
            </p>
      ...
      {/* Right: giant X watermark SVG (desktop only) */}
