/**
 * Messages page — route: /messages
 *
 * Full direct messaging interface with:
 *   - Left panel: conversation list with unread counts and last message previews
 *   - Right panel: chat view with polling (2s interval) for new messages
 *   - "New message" modal to search users and start conversations
 *   - Auto-mark conversations as read on open
 *   - Responsive: single column on mobile, two columns on desktop (lg+)
 *
 * The conversationId is read from the URL query string (?conversationId=xxx)
      ...
      <div className="bg-black border border-neutral-700 rounded-2xl w-full max-w-lg mx-4">
        ...
        <div className="p-4">
          <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search people"
            className="w-full bg-neutral-900 text-white rounded-full px-4 py-2.5 text-[15px] outline-none focus:ring-1 focus:ring-[rgb(29,155,240)] placeholder-neutral-500" autoFocus />
        </div>
        {search.length >= 2 && (
          <div className="max-h-60 overflow-y-auto border-t border-neutral-800">
            ...
            {users?.map((u) => (
              <button key={u.id} onClick={() => createConv.mutate({ participantId: u.id })} disabled={createConv.isPending}
                ...
                <div className="text-left"><p className="font-bold text-[15px]">{u.name}</p><p className="text-neutral-500 text-[13px]">@{u.username}</p></div>
              </button>
            ))}
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="sticky top-0 bg-black z-10 border-b border-neutral-800 px-4 py-3 flex items-center justify-between">
        ...
        <button onClick={() => setShowNew(true)} className="p-2 rounded-full hover:bg-neutral-900 transition-colors"><Plus size={20} /></button>
      </div>
      {showNew && <NewMessageModal onClose={() => setShowNew(false)} />}
      {isLoading && <div className="flex items-center justify-center py-8"><Loader2 className="animate-spin text-neutral-500" size={24} /></div>}
      {conversations?.length === 0 && <div className="p-4 text-neutral-500 text-center">No conversations yet.</div>}
        ...
        {conversations?.map((conv) => {
          ...
          return (
            <button key={conv.id} onClick={() => onSelect(conv.id)}
              ...
              <div className="h-10 w-10 rounded-full bg-neutral-700 flex-shrink-0 flex items-center justify-center overflow-hidden">
                {other?.image ? <img src={other.image} alt="" className="w-full h-full object-cover" width={40} height={40} /> : <User size={20} className="text-neutral-400" />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-baseline justify-between">
                  <span className="font-semibold text-[15px] truncate">{other?.name ?? "Unknown"}</span>
                  {conv.lastMessage && <span className="text-neutral-500 text-[13px] flex-shrink-0 ml-2">{timeAgo(conv.lastMessage.createdAt)}</span>}
                </div>
                <span className="text-neutral-500 text-[15px] truncate block">
                  {conv.lastMessage ? <>{conv.lastMessage.senderId === session?.user.id && "You: "}{conv.lastMessage.content}</> : "No messages yet"}
                </span>
              </div>
              {conv.unreadCount > 0 && (
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="sticky top-0 bg-black z-10 border-b border-neutral-800 px-4 py-3 flex items-center gap-3">
        ...
        <span className="font-bold text-[17px]">{messages && messages.length > 0 ? messages[0]?.sender.name ?? "Chat" : "Chat"}</span>
      </div>
            ...
            <div key={msg.id} className={`flex ${msg.isOwn ? "justify-end" : "justify-start"}`}>
              <div className={`max-w-[75%] px-4 py-2 rounded-2xl text-[15px] leading-normal whitespace-pre-wrap break-words ${msg.isOwn ? "bg-[rgb(29,155,240)] text-white rounded-br-md" : "bg-neutral-800 text-white rounded-bl-md"}`}>
                <div className="flex items-end gap-1">
                  ...
                  <span className={`text-[11px] whitespace-nowrap flex-shrink-0 ${msg.isOwn ? "text-blue-200" : "text-neutral-400"}`}>{formatTime(msg.createdAt)}</span>
                </div>
              </div>
            </div>
          ))}
